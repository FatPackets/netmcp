...
DEFAULT_TIMEOUT = 30
DEFAULT_PORT = 22
COMMAND_OVERRIDE_ENV = "NETMCP_COMMAND_OVERRIDES"
DEFAULT_OVERRIDE_FILE = "netmcp.commands.json"
...
INVENTORY_COMMANDS: Dict[str, Dict[str, str]] = {
    ...
    "checkpoint": {
        ...
        "interfaces": "show interfaces all",
        "routing":    "show route",
        "neighbors":  "show arp dynamic all",
    },
    "arista_eos": {
        "version":    "show version",
        "interfaces": "show interfaces",
...
PING_COMMANDS: Dict[str, str] = {
    ...
    "cisco_ftd":    "ping {target}",
    "cisco_xr":     "ping {target} count {count}{source}{vrf}",
    ...
    "paloalto_panos":"ping host {target} count {count}{source}",
}
...
CONFIG_COMMANDS: Dict[str, Dict[str, str]] = {
    ...
    "cisco_ftd":    {"running": "show running-config", "startup": "show startup-config"},
    "cisco_xr":     {"running": "show running-config", "startup": "show startup-config"},
    "f5_tmsh":      {"running": "list", "startup": "list"},
    "checkpoint":   {"running": "show configuration", "startup": "show configuration"},
    ...
    "paloalto_panos":{"running": "show config running", "startup": "show config candidate"},
}


def _apply_inventory_overrides(overrides: Dict[str, Any]) -> int:
    if not isinstance(overrides, dict):
        logger.warning("inventory overrides must be an object")
        return 0

    applied = 0
    valid_sections = {"version", "interfaces", "routing", "neighbors"}

    for vendor, section_map in overrides.items():
        if vendor not in SUPPORTED_VENDORS:
            logger.warning("Ignoring inventory override for unsupported vendor '%s'", vendor)
            continue
        if not isinstance(section_map, dict):
            logger.warning("Ignoring inventory override for vendor '%s': expected object", vendor)
            continue

        current = dict(INVENTORY_COMMANDS.get(vendor, {}))
        for section, command in section_map.items():
            if section not in valid_sections:
                logger.warning("Ignoring inventory section '%s' for vendor '%s'", section, vendor)
                continue
            if not isinstance(command, str) or not command.strip():
                logger.warning("Ignoring inventory command for '%s.%s': expected non-empty string", vendor, section)
                continue
            current[section] = command.strip()
            applied += 1

        INVENTORY_COMMANDS[vendor] = current

    return applied


def _apply_ping_overrides(overrides: Dict[str, Any]) -> int:
    if not isinstance(overrides, dict):
        logger.warning("ping overrides must be an object")
        return 0

    applied = 0
    for vendor, command in overrides.items():
        if vendor not in SUPPORTED_VENDORS:
            logger.warning("Ignoring ping override for unsupported vendor '%s'", vendor)
            continue
        if not isinstance(command, str) or not command.strip():
            logger.warning("Ignoring ping override for vendor '%s': expected non-empty string", vendor)
            continue
        PING_COMMANDS[vendor] = command.strip()
        applied += 1

    return applied


def _apply_config_overrides(overrides: Dict[str, Any]) -> int:
    if not isinstance(overrides, dict):
        logger.warning("config overrides must be an object")
        return 0

    applied = 0
    valid_types = {"running", "startup"}

    for vendor, config_map in overrides.items():
        if vendor not in SUPPORTED_VENDORS:
            logger.warning("Ignoring config override for unsupported vendor '%s'", vendor)
            continue
        if not isinstance(config_map, dict):
            logger.warning("Ignoring config override for vendor '%s': expected object", vendor)
            continue

        current = dict(CONFIG_COMMANDS.get(vendor, {}))
        for config_type, command in config_map.items():
            if config_type not in valid_types:
                logger.warning("Ignoring config type '%s' for vendor '%s'", config_type, vendor)
                continue
            if not isinstance(command, str) or not command.strip():
                logger.warning("Ignoring config command for '%s.%s': expected non-empty string", vendor, config_type)
                continue
            current[config_type] = command.strip()
            applied += 1

        CONFIG_COMMANDS[vendor] = current

    return applied


def _resolve_override_file() -> Optional[Path]:
    env_path = os.getenv(COMMAND_OVERRIDE_ENV, "").strip()
    if env_path:
        return Path(env_path).expanduser().resolve()

    default_path = (Path.cwd() / DEFAULT_OVERRIDE_FILE).resolve()
    if default_path.exists():
        return default_path
    return None


def _load_command_overrides() -> Dict[str, Any]:
    override_file = _resolve_override_file()
    ...
    if not override_file.exists():
        logger.warning("Command override file not found: %s", override_file)
        return {
            "loaded": False,
            "source": str(override_file),
            "applied": 0,
    ...
    try:
        ...
        logger.warning("Invalid JSON in command override file %s: %s", override_file, e)
        return {
            "loaded": False,
            "source": str(override_file),
            "applied": 0,
        ...
        logger.warning("Unable to read command override file %s: %s", override_file, e)
        return {
            "loaded": False,
            "source": str(override_file),
            "applied": 0,
    ...
    if not isinstance(payload, dict):
        logger.warning("Command override file must contain a JSON object at top level")
        return {
            "loaded": False,
            "source": str(override_file),
            "applied": 0,
            "error": "invalid_top_level",
        }

    applied = 0
    applied += _apply_inventory_overrides(payload.get("inventory", {}))
    applied += _apply_ping_overrides(payload.get("ping", {}))
    applied += _apply_config_overrides(payload.get("config", {}))

    logger.info("Loaded command overrides from %s (%s entries applied)", override_file, applied)
    return {
        "loaded": True,
        "source": str(override_file),
        "applied": applied,
...
COMMAND_OVERRIDE_STATUS = _load_command_overrides()
...
@asynccontextmanager
async def app_lifespan(app):  # <-- accept the arg (name doesn't matter)
    logger.info("═" * 60)
    ...
    if COMMAND_OVERRIDE_STATUS["source"]:
        logger.info(f"  overrides file    : {COMMAND_OVERRIDE_STATUS['source']}")
        logger.info(f"  overrides applied : {COMMAND_OVERRIDE_STATUS['applied']}")
    logger.info("═" * 60)
    yield {}
...
async def net_vendors() -> str:
    ...
    return json.dumps({
        ...
        "command_overrides": COMMAND_OVERRIDE_STATUS,
